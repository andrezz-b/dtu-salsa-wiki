...
const generateSlug = (title: string) => slugger.slug(title);
...
function scanDirectory(dir: string, type: 'move' | 'concept') {
  ...
  for (const file of files) {
    if (file.endsWith('.md')) {
      ...
      // Also map the slug back to the info for easier lookup if needed, or just rely on title
      // We might need case-insensitive lookup? Obsidian is usually case-sensitive but links might vary.
      // For now, assume exact match or handle simple variations.
    }
  }
}
...
for (const [title, info] of fileMap.entries()) {
  ...
  if (data.created_date) {
    // Format: "YYYY-MM-DD, H:mm" -> ISO
    // Simple parse assuming the format is consistent
    try {
        const [datePart, timePart] = data.created_date.split(', ');
        newFrontmatter.created_date = new Date(`${datePart}T${timePart}`);
    } catch (e) {
        console.warn(`Failed to parse created_date for ${title}: ${data.created_date}`);
    }
  }
  if (data.updated_date) {
    try {
        const [datePart, timePart] = data.updated_date.split(', ');
        newFrontmatter.updated_date = new Date(`${datePart}T${timePart}`);
    } catch (e) {
        console.warn(`Failed to parse updated_date for ${title}: ${data.updated_date}`);
    }
  }

  // Resolve Relations (convert titles to slugs)
  const resolveRelations = (items: string[] | undefined) => {
    if (!items) return undefined;
    const slugs = new Set<string>();
    for (const item of items) {
      // Clean up [[Link]] syntax if present in frontmatter (sometimes happens)
      const cleanItem = item.replace(/^\[\[(.*)\]\]$/, '$1');
      const target = fileMap.get(cleanItem);
      if (target) {
        slugs.add(target.slug);
      } else {
        // If not found, maybe keep original or warn?
        // For now, keep original but slugified as a fallback or just warn
        console.warn(`Relation not found: ${cleanItem} in ${title}`);
        // slugs.add(generateSlug(cleanItem)); // Optional: auto-slugify unknown refs
      }
    }
  ...
  // Transform Content
  // 1. Replace [[Link]] with [Link](/path/slug)
  // 2. Collect referenced items to add to frontmatter if missing
  ...
  body = body.replace(/\[\[([^\]]+)\]\]/g, (match, linkText) => {
    ...
    if (targetInfo) {
      ...
      return match; // Keep as is or convert to text?
    }
  });
  ...
  if (referencedMoves.size > 0) {
    ...
    for (const move of referencedMoves) {
        if (move !== info.slug) existing.add(move); // Don't link to self
    }
  ...
  const cleanFrontmatter = (obj: any): any => {
